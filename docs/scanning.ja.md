...
**目次：** [始める前に](#始める前に) · [カメラ側の機材](#カメラ側の機材) · [撮影倍率とレンズの選択](#撮影倍率とレンズの選択) · [カメラの高さとスタンド](#カメラの高さとスタンド) · [平行出し](#平行出し) · [ピント](#ピント) · [露出](#露出) · [フィルムの装填](#フィルムの装填) · [マウント済みスライド](#マウント済みスライド) · [4×5 シート](#45-シート) · [1 本撮る](#1-本撮る) · [フラットフィールド補正と反転](#フラットフィールド補正と反転) · [おかしいと感じたら](#おかしいと感じたら)
...
センサーをコマで埋めるために必要な撮影倍率を、レンズを選ぶための概算として示します。
...
1:1 のマクロレンズがあれば、この表のすべての行をカバーできます。大きいフォーマットでは、単に等倍より低い倍率を使うだけです。いちばん厳しいのはフルサイズで 135 を撮る場合で、ここだけは 1.0× をまるごと必要とします。
...
フィルム面は、**箱が載っている面から 83.2 mm 上**に来ます。この高さは設計で固定されていて、135 でも 120 でも同じです。どちらのフィルムホルダーもフィルムを同じ高さに差し出すので、フォーマットを替えても水平出しをやり直す必要はありません。例外は小さなものです。6×6 のマスクは 120 のホルダー全体を 1 mm 持ち上げ、マウント済みスライドは約 82.6 – 83.6、4×5 のシートは約 82.2 に来ます。正常な動作なので、ピントを合わせ直せば済みます。4×5 で大きく変わるのはワーキングディスタンスです。100 mm マクロを 0.25× で使うと前玉はフィルムからおよそ半メートル上に来るので、ポールが届くか確認するか、50 – 60 mm のマクロにしてその半分に抑えてください。
...
1. フィルムのディテールが見える程度の弱い室内光を用意します。ただし、どの照明もオープンフロントに直接差し込まないこと。
...
1. 出発点として、ストロボを 1/8 に設定します。マニュアル発光できるクリップオンストロボなら何でも使えます。参考機の TT560 の[マニュアル発光量 (manual power fraction)](glossary.ja.md#マニュアル発光量-manual-power-fraction) は 1/1 から 1/128 まで 1 段刻みで、8 段階しかなく、その中間はありません。
...
裸のストロボを測ったときの値に対して、箱の中の反射経路のぶん、数段の発光量を余計に払うことになると見込んでください。この設計には損失の実測値がありません。まだ作られていないからです。1/8 はあくまで出発点として、ブラケットで探してください。
...
1. 押さえエレメントをブロアーで払ってから、ホルダー底の 4.6 mm のエレメント段に収めます。ガラスモードでは特に下面を払います。フィルム面からわずか 0.2 mm なので、そこのホコリはそのまま画面に写ります。
...
- **フォーマットの変更＝ホルダーの交換。** ホルダー全体をマグネットに逆らって持ち上げて外し、もう一方を収めるだけです。数秒で済み、水平出しのやり直しは何も要りません。
- **縁を持って扱う。** 清潔な手か木綿の手袋で。
- **6×6** は 120 のホルダーを使い、`mask-6x6.stl` をホルダー底の**下**、トレイの中に敷きます。ホルダー全体が 1 mm 高くなりますが、正常です。ピントを合わせ直してください。**6×4.5** には専用マスクがありません。120 の窓の中でフレーミングし、後処理でトリミングします。
- **マウント済みのスライド**はホルダーではなく、スライドプレートに入れます。[マウント済みスライド](#マウント済みスライド)を参照してください。

<details>
<summary>なぜフィルムが平らに保たれるのか、そしてなぜ必ず後処理でトリミングするのか</summary>

フィルムの縁はホルダー底の受け段に載り、その上面は押さえエレメントの下面より 0.4 mm 低く、つまり四辺とコマの全幅にわたって機械的に規制された通路です。インサートでは、画面領域には何ひとつ触れず、うねりは最大でも約 0.28 mm で、1:1・f/8 での約 ±0.4 mm の被写界深度が覆います。アンチニュートンガラスでは、コマ全面が連続的に押さえられ、どの位置も同じ 0.28 mm 以内に保たれますが、それでもエマルジョンには何も触れません。ガラスのつや消し面がつやのあるベース面の上を滑る組み合わせでは[ニュートンリング (Newton rings)](glossary.ja.md#ニュートンリング-newton-rings) が出ず、エマルジョン面は下の開いた窓を向いているからです。
...
マウントの厚みはおよそ 1.2 – 3.2 mm、溝は 0.4 mm なので、マウント済みのスライドがホルダーに入ることはありません。専用の部品 `slide-plate-135.stl` があります。135 のホルダー 1 組の代わりにトレイに立つ板で、51.4 mm 角のポケットと、その下の 26 × 38 mm の窓を持ちます。蓋もマグネットも押さえエレメントもありません。マウントは硬く、箱は水平なので、ポケットが位置を決め、重力が押さえます。

1. 135 のホルダーをトレイから持ち上げて外し、スライドプレートを収めます。
   *チェックポイント：* プレートがフランジの内側に平らに座り、がたつかないこと。
2. ポケットと、スライドの両面をブロアーで払います。
3. マウントを、窓にかざして見るときと同じ向きでポケットに落とし込みます。上から見て像が正しく読める向き、つまりエマルジョン面が下です。
   *チェックポイント：* マウントがポケットの底に座り、窓を完全に覆っていること。
4. ピントを合わせ直します。マウントの中のフィルムは約 82.6 – 83.6 mm にあり、ストリップのフィルム面 83.2 とは違ううえ、紙マウントとプラマウントでも異なります。マウントの種類ごとに一度、粒子にピントを合わせてください。
5. ポケットの両側にある 2 つの指掛け溝に指を掛けてマウントを取り出し、次を落とし込みます。

反った紙マウントや、プロジェクターで飛び出したコマは、ここでは何も平らにしません。専用のスライドスキャナーと同じです。1:1・f/8 での約 ±0.4 mm の被写界深度が、そのほとんどを吸収します。ガラスマウントはそのまま入れます。窓はどのマウントの開口よりも大きいので、画面を縁取るのはマウントです。後処理でそこまでトリミングしてください。

## 4×5 シート

4×5 にはオプションの 4×5 ステージが要ります。天板ステージの代わりの `cover-stage-4x5.stl`、その溝に入る専用の 106 × 130 × 2 の乳白板、そのトレイに立つ `sheet-plate-4x5.stl` です。シートプレートは 101.6 × 127 のシートを 102.2 × 127.6 のポケットで受け、97 × 121 の窓の上に置きます。本体、ストロボ、スタンドは変わりません。

1. 天板ステージをフランジをつかんで持ち上げ、アクリルを溝に入れた 4×5 天板ステージを同じ 4 つのダボに載せます。
   *チェックポイント：* 天板ステージとまったく同じように、自重でダボに落ち着くこと。
2. シートプレートをトレイに収めます。
   *チェックポイント：* フランジの内側で平らに座り、がたつかないこと。
3. ポケットとシートの両面を吹き、シートを窓にかざして見るときと同じ向き、つまり上から見て像が正しく読める向き（エマルジョン面が下）でポケットに落とし込みます。
   *チェックポイント：* シートがポケットの底に載り、窓が完全に覆われていること。
4. ピントを合わせ直し、ポールの高さも取り直します。フルサイズで 4×5 はおよそ 0.25×、シートは約 82.2 mm に来ます。0.25×・f/8 の被写界深度はおよそ 8 – 10 mm あるので、反ったシートもピントの問題にはなりません。
5. シートは 2 つの縁の切欠きに指を掛けて取り出します。

**唯一の未知数は均一性です。** 4×5 の窓はチャンバーの壁から 9 と 12 mm で、v1 の 29 と 27.5 より近くにあります。まず何も入れずに 1 コマ撮って、よく見てください。受け入れられない勾配が見えたら、[フラットフィールド補正](#フラットフィールド補正と反転)が後処理で取り除きます。マージンを取り戻す大きな箱は[設計 §10](design.ja.md#10-v1-本体での-45)にあります。
...
- **1 本の途中で発光量を変えない。** ストロボは机の上にあって、つい手が届いてしまいますが、我慢してください。一度変えれば、1 本 1 プロファイルという利点が壊れます。
...
この 2 つの仕事をこなすソフトウェアを挙げます。実在して広く使われているという理由によるもので、推奨ではありません。
...
| 1 本のあいだに露出が流れていく | 発光量が動いたか（ストロボは机の上にむき出しです）、または電池が消耗している |
| フラットフィールド補正後も隅のムラが残る | レンズではなく光源。発光部をオープンフロントに付け直して、フラットフレームを撮り直す |

プリントや組立の不具合が撮影の問題として現れるものも含めて、症状・原因・対処の完全な表は[トラブルシューティング](troubleshooting.ja.md#撮影)にあります。
