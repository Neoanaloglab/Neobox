...
> NeoBox の 12 点のパーツをどう出力するか。なぜほぼどのプリンタでも出せるのか、ビルド全体で共通の設定 1 組、ファイルごとのカード、そして組み立てに入る前に走らせる確認項目をまとめます。

**目次：** [お使いのプリンタで出力できるか](#お使いのプリンタで出力できるか) · [プリント設定](#プリント設定) · [フィラメント](#フィラメント) · [12 点のパーツ](#12-点のパーツ) · [まず 135 ホルダーを 1 組だけ出力する](#まず-135-ホルダーを-1-組だけ出力する) · [組み立てる前に各パーツを確認する](#組み立てる前に各パーツを確認する) · [きつい場合と緩い場合の対処](#きつい場合と緩い場合の対処) · [出力サービスへの発注](#出力サービスへの発注)

STL は 12 ファイルで、**標準ビルドの 8 点＋オプション 4 点（6×6 マスク、マウント済みスライド用のスライドプレート、そして 4×5 の天板ステージとシートプレート）**です。うち白が 3 点、黒が 9 点。すべて 1:1 のミリメートル・モデルで、すべてサポートなしで出力でき、フィラメントは標準ビルドでおよそ **300–350 g、プロトタイプのほぼ 3 分の 1** です。
...
> [!NOTE]
> この設計はまだ出力されていません。このページの記述はいずれも実機から得たものではありません。形状は CAD で検証済みです（権威は `cad/neobox.blend`）。

---

## お使いのプリンタで出力できるか

ほぼ間違いなく出力できます。ビルド最大のパーツである本体と天板ステージは、プレート上で 124.8 × 154.8 mm、最も高いのは 75.6 mm の本体です。**160 × 160 mm のビルドプレートがあればセット全体を出力できます**。つまり現行の小型機まで含めて、ほとんどすべてのデスクトップ機で足ります。プロトタイプ時代の大型パーツと筐体外注に関する警告は、プロトタイプとともに廃止されました。
...
どの数値も 0.2 の整数倍です。そしてビルド中で最も細かい露出段差、すなわちホルダー底の 0.4 mm の受けとエレメント段こそが、フィルム面を定義する 2 段です。だから仕様は全ファイルの積層ピッチを 0.2 mm に固定しています：どの面も層境界にちょうど乗り、0.4 の段差はきれいな 2 層になり、スライサーの丸めに委ねるものがなくなります。0.28 のような粗いピッチはわずかに安くはなりますが、グリッドが層境界の間に落ち、ぼやけるのはフィルム面を支えるまさにその段差です。
...
**筐体 3 点、本体とどちらの天板ステージも白。** 本体の内側そのものが反射面です。ストロボは完全に開いた前面（オープンフロント）から腔内に発光し、白い壁と天板ステージの白い裏面の間で何度も反射してからフィルムに届きます。白い生地の面が光学面**そのもの**です。

> [!IMPORTANT]
> 白は通常の白 PLA を使い、シルク・サテンなどの特殊仕上げは避けてください。内側の面には手を入れないこと。塗装・研磨・コーティングはいずれも不可です。白壁の生地のままの反射率は光学設計の一部です。

**フィルムの近くにある 9 点は黒。** フィルムが至近距離で「見る」もの（ホルダー底とホルダー蓋、押さえ窓インサート、マスク、スライドプレート、シートプレート）はすべて黒で、迷光を吸収します。

**数量。** 標準ビルドでおよそ **300–350 g**、プロトタイプのほぼ 3 分の 1 です。オプションのスライドプレートは約 25 g、4×5 の 2 点は約 35 g 加わります。各色 1 巻きで何度でも作れる量です。
...
| ポケット | スライドプレートの上面にある 51.4 mm 角のくぼみ。マウント済みスライドをここに落とし込む |
| 指掛け溝 | ポケットの両側にある小さな貫通しないくぼみの一つ。マウントを取り出すときに指を掛ける |
| 縁の切欠き | シートプレートの短辺にある 2 つの開いた切り込みの一つ。シートを取り出すときに指を掛ける |
...
- **プレート上の姿勢：** 平らな底面を下に。3 面の壁が底から立ち上がり、4 面目の完全に開いた前面には壁がそもそもありません。これは設計であって、壊れたファイルではありません。4 個の位置決めダボは側壁の上面に載っています。
...
- **白。** 124.8 × 154.8 × 10.0。この 1 点がプロトタイプの天板とフィルムステージを統合したパーツです。
...
- **サポート：不要。**
- **注意：** 135 の底と同じで、受けとエレメント段がフィルム面です。
...
### [slide-plate-135.stl](../stl/black-pla/slide-plate-135.stl)——オプション

- **黒。** 94 × 120 × 5.0。51.4 × 51.4・深さ 2.0 の角ポケット、ポケットの底を貫通する窓 26 × 38、指掛け溝 2 つ。
- **プレート上の姿勢：** 平らな面を下に。ポケットと指掛け溝が上を向きます。
- **サポート：不要。** くぼみはすべて上に開き、窓は貫通穴です。
- **出力するのは** マウント済みの 135 スライドを持っている場合だけです。135 のホルダー 1 組の代わりにトレイに立ち、マウントをポケットに落とし込みます。蓋もマグネットも押さえエレメントもありません。

### [cover-stage-4x5.stl](../stl/white-pla/cover-stage-4x5.stl)——オプション

- **白。** 124.8 × 154.8 × 10.0。天板ステージと同じ外形に、102 × 126 の採光窓、106 × 130 × 2 の乳白板を収める溝、112.6 × 138.6 のトレイ。ワッシャーポケットはありません。
- **プレート上の姿勢：** 天板ステージとまったく同じ。四隅にダボ穴のある平らな面を下に、立ち上がったトレイフランジを上に。
- **サポート：不要。** ダボ穴の天井は天板ステージと同じです。
- **出力するのは** 4×5 を撮る場合だけです。同じ本体の上で天板ステージの代わりに載ります。裏面はチャンバーに面する光学面なので、研磨も塗装も不可です。

### [sheet-plate-4x5.stl](../stl/black-pla/sheet-plate-4x5.stl)——オプション

- **黒。** 112 × 138 × 5.0。102.2 × 127.6・深さ 2.0 のポケット、ポケットの底を貫通する窓 97 × 121、短辺に縁の切欠き 2 つ。
- **プレート上の姿勢：** 平らな面を下に。ポケットと切欠きが上を向きます。
- **サポート：不要。**
- **出力するのは** 4×5 を撮る場合だけです。4×5 天板ステージのトレイに立ち、シートをポケットに落とし込みます。蓋もマグネットも押さえエレメントもありません。
...
- [ ] 出力した場合：`slide-plate-135.stl` が 94 × 120 で、トレーに収まり、マウント済みスライドがポケットに落ちて指掛け溝から取り出せること。
- [ ] 出力した場合：`cover-stage-4x5.stl` が天板ステージと同じように 4 つのダボに落ち着き、`sheet-plate-4x5.stl` が 112 × 138 でそのトレイに収まること。
...
プリンタがまったくなくても、これはもう小さな注文です。12 ファイル（うち 4 点は任意）、通常の PLA がおよそ 300–350 g、サポートは一切なし、どのパーツも 160 × 160 mm のビルドプレートに収まります。検索ワードは `3Dプリント 出力代行 PLA`。次の文面をそのまま送ってください。全ファイル名を挙げ、オペレーターの画面で見える特徴で置き方を指定してあります。

```text
STL 12 ファイル：必須 8 点＋任意 4 点（mask-6x6、slide-plate-135、cover-stage-4x5、sheet-plate-4x5）。
単位はミリ、1:1。スケール変更は不可。
...
白 PLA - 2 点 + 任意 1 点:
  ...
  cover-stage.stl      124.8 x 154.8 x 10。
                       ...
                       長方形の枠を上に。
  cover-stage-4x5.stl（任意）  cover-stage.stl と同じ配置。
...
黒 PLA - 6 点 + 任意 3 点:
  ...
  slide-plate-135.stl（任意）  平らな面をプレートに、角ポケットを上に。
  sheet-plate-4x5.stl（任意）  平らな面をプレートに、ポケットを上に。
```

**支払う前に 3 つ質問してください。** 出力サービスは先に注文を受けてから問題に気づきます。

1. 全ファイルを 100 % スケール・ミリ単位で出力しますか。「プレートに合わせて縮小」はしませんか。
2. 白のフィラメントは、シルクでも光沢でもない通常の白 PLA ですか。
3. 2 点のホルダー蓋を、文面どおり天面を下にして配置してもらえますか。

**ショップが設定に難色を示したら、**先方のデフォルトプロファイルに任せて構いません。デフォルトで通るように形状を量子化してあります。すべての水平面が 0.2 mm グリッドに乗り、露出段差は 0.4 mm 以上です。譲れないのは「スケールしない」「姿勢は文面どおり」「サポートなし」の 3 点だけです。
