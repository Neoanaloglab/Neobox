...
**目次：** [1. 光路](#1-光路) · [2. 寸法チェーン](#2-寸法チェーン) · [3. 光学上の判断](#3-光学上の判断) · [4. 天板ステージ](#4-天板ステージ) · [5. フィルムホルダー](#5-フィルムホルダー) · [6. 筐体](#6-筐体) · [7. フォーカス用ライト](#7-フォーカス用ライト) · [8. ストロボの運用](#8-ストロボの運用) · [9. 位置合わせと再現性](#9-位置合わせと再現性) · [10. v1 本体での 4×5](#10-v1-本体での-45) · [11. ソースファイルを扱う](#11-ソースファイルを扱う)
...
STL は 12 ファイル。白 3 点、黒 9 点、すべてサポート不要です：
...
| `slide-plate-135.stl` | 黒 | 94 × 120 × 5 | マウント済み 135 スライド用のスライドプレート：51.4 × 51.4 のポケット、窓 26 × 38、指掛け溝 2 つ。1 部品で完結する 4 組目のホルダー（[§5](#5-フィルムホルダー)） |
| `cover-stage-4x5.stl` | 白 | 124.8 × 154.8 × 10.0 | 4×5 天板ステージ：天板ステージと同じ外形・板・4 つのダボ穴に、採光窓 102 × 126、アクリル溝 107 × 131、トレイ 112.6 × 138.6 を持つ。ワッシャーポケットはない（[§10](#10-v1-本体での-45)） |
| `sheet-plate-4x5.stl` | 黒 | 112 × 138 × 5 | 4×5 シートプレート：102.2 × 127.6 のポケット、窓 97 × 121、縁の切欠き 2 つ。4×5 天板ステージのトレイに立つ |
...
| ホルダー底 | 79.0 – 84.0 | フランジの内側、デッキの上に立つ。スライドプレートも同じ 5 mm を占める |
| **フィルム面** | **83.2** | 受けの上面。どちらのフォーマットでも同じ高さ |
| スライドのフィルム面 | ≈ 82.6 – 83.6 | スライドプレート内：ポケットの底 82.0 にマウント厚のおよそ半分を足した高さ。ピントを合わせ直す |
| 4×5 のフィルム面 | ≈ 82.2 | シートプレートのポケットの底 82.0 に載ったシート。ピントを合わせ直す |
...
| スライドプレート | 0 · 1.0 指掛け溝の底 · 3.0 ポケットの底 · 5.0 上面 |
| 4×5 天板ステージ | 0 · 2.8 ダボ穴の天井 · 3.6 アクリル溝の底 · 6.0 デッキ · 10.0 フランジの上面 |
| シートプレート | 0 · 1.0 切欠きの底 · 3.0 ポケットの底 · 5.0 上面 |
...
フォーマットごとにホルダー 1 組。ホルダー底とホルダー蓋、どちらも 94 × 120 で、天板ステージのトレイに立ちます。フォーマットの変更は、1 組を持ち上げてもう 1 組を下ろすだけ：マグネットが外れて吸い直すまで 5 秒ほどで、ほかには何も動きません。コマ送りは、閉じたままのホルダーからフィルムを引き抜く動作で、1 本の途中でホルダーを開けることはありません。マウント済み 135 スライド用の 4 組目は、蓋のない 1 枚の板です（[下記](#マウント済みスライドスライドプレート)）。
...
マウントに入ったスライドの厚みはおよそ 1.2 – 3.2 mm で、0.4 mm の通り道の何倍もあるので、135 ホルダーには決して入りません。かといって 135 の底にポケットを掘れば、ストリップを支える受けがなくなります（[設計ログのエントリ 25](design-log.ja.md#25-マウント済みスライドには専用の板を)）。そこで `slide-plate-135.stl` を、1 部品で完結する 4 組目のホルダーとしました：94 × 120 × 5 の黒い板 1 枚で、135 のホルダー 1 組の代わりにトレイに立ちます。蓋もマグネットも押さえエレメントもありません。マウントは硬く、箱は水平なので、ポケットが位置を決め、重力が押さえます。

| 項目 | スライドプレート |
|---|---|
| 外形と厚み | 94 × 120 × 5。トレイの座はホルダー底と同じ |
| ポケット | 51.4 × 51.4、深さ 2.0。底は板の底面から 3.0（z = 82.0） |
| 窓 | 26 × 38、ポケットの底を貫通 |
| 指掛け溝 | 2 つ、18 × 12、ポケットの短辺の中央。底は 1.0 で、貫通しない |
| フィルム面 | ポケットの底にマウント厚のおよそ半分を足した高さ：紙マウントで約 82.6、ガラスマウントで約 83.6。ピントを合わせ直す |

**なぜ 51.4 か：** マウントの公称は 50 × 50 ですが、紙マウントは 50.8（2 インチ）まであります。51.4 なら最大のマウントでも片側 0.3 mm、プラマウントなら 0.7 mm の隙間が残り、ホルダー底がトレイに持つ隙間と同じです。**なぜ深さ 2.0 か：** いちばん薄い紙マウント（約 1.2）でもポケットの壁が捕まえ、いちばん厚いガラスマウント（約 3.2）は 1.2 突き出しますが、上には何もないので邪魔になりません。**なぜ 25 × 37 ではなく 26 × 38 か：** マウントはポケットの中で最大 0.7 mm 動けます。公称 24 × 36 のコマに対して片側 1 mm の余裕があれば、画面を縁取るのは常にマウント自身の開口であって、プリントの縁ではありません。それでもどのマウントも窓を四辺とも 5 mm 以上覆うので、光は漏れません。**指掛け溝**は、ポケットの縁より 0.8 低く座る紙マウントの縁に爪を掛けるためのものです。貫通せず、ポケットの底より 2.0 深いだけなので、光は通しません。

マウントは、窓にかざして見るときと同じ向きに入れます：上から見て像が正しく読める向き、つまりエマルジョン面が下。ストリップと同じ規則です。反った紙マウントや飛び出したコマは、ここでは何も平らにしません。専用のスライドスキャナーと同じです。1:1・f/8 での約 ±0.4 mm の被写界深度が、そのほとんどを吸収します。
...
フィルム面は**どちらのストリップ判型でも** z = 83.2 に固定です（6×6 マスクは 120 の 1 組を 1 mm 持ち上げ、スライドプレートとシートプレートはフィルムをそこから 1 mm 以内に置きます。ピントを合わせ直すだけで、ほかは何も変わりません）。135 と 120 で変わるのは必要な撮影倍率、したがってカメラの高さであって、フィルムの高さではありません。カメラの高さは、フィルム面の高さにレンズの[ワーキングディスタンス](glossary.ja.md#ワーキングディスタンス-working-distance)を足して求めます。計算は[カメラの高さとスタンド](scanning.ja.md#カメラの高さとスタンド)にあります。
...
4×5 は、本体をそのままに、オプションの部品 2 点と一回り大きいアクリル 1 枚で載ります。`cover-stage-4x5.stl` が天板ステージの代わりに載り、そのトレイに `sheet-plate-4x5.stl` が立ち、溝には 106 × 130 × 2 の乳白板が入ります。ほかは何も変わりません。同じダボ、同じオープンフロントに同じストロボ、同じ重力スタック、同じ 160 × 160 のビルドプレートです。

| 項目 | 4×5 天板ステージ | v1 天板ステージ |
|---|---|---|
| 外形・板・ダボ穴 | 124.8 × 154.8、6 mm の板、ダボに掛かる 4 つのダボ穴 | 同一 |
| 採光窓 | 102 × 126 | 62 × 95 |
| アクリル溝 | 107 × 131、底は 3.6。106 × 130 × 2 の板用 | 69 × 119。68 × 118 × 2 用 |
| フランジ内側のトレイ | 112.6 × 138.6、フランジ幅 4、上面 10.0 | 94.6 × 120.6、フランジ幅 7 |
| ワッシャーポケット | なし：シートプレートにマグネットはない | 4 個 |
| 拡散マージン（窓の縁から壁まで） | x 方向 9、y 方向 12 | x 方向 29、y 方向 27.5 |

| 項目 | シートプレート |
|---|---|
| 外形と厚み | 112 × 138 × 5。4×5 のフランジの内側でデッキの上に立つ |
| ポケット | 102.2 × 127.6、深さ 2.0。底は板の底面から 3.0（z = 82.0） |
| 窓 | 97 × 121、ポケットの底を貫通。画面 96 × 120 に、いつもの片側 0.5 を足したもの |
| 縁の切欠き | 2 つ、幅 18、短辺側。シートの縁の 3.7 内側から板の縁を突き抜けて外まで。底は 1.0 |
| フィルム面 | シートはポケットの底に載る：約 82.2 |

**なぜシートに押さえエレメントが要らないのか。** フルサイズで 4×5 を撮る倍率はおよそ 0.25× で、そのとき f/8 の被写界深度はおよそ 8 – 10 mm、135 の場合の 10 倍です。シート自身のミリ単位の反りは、この尺度では見えません。だからこの板は、シートの縁を受ける受けを持つポケットであって、上には何も載りません。蓋もガラスもマグネットもなし。シートは 1 枚ずつ、上から見て像が正しく読める向き（エマルジョン面が下）で入れ、縁の切欠きから取り出します。

**分かっていないこと：均一性。** v1 の窓は壁から 29 と 27.5 mm、4×5 の窓は 9 と 12 mm です。マージンの下限はこれまで一度も確立されておらず、何も測っていません。だから 4×5 ステージは、数値としてではなく、同じ本体の上での実験として公開します。まず何も入れずに 1 コマ撮ってください。受け入れられない勾配が見えたら、後処理のフラットフィールド補正が第一の手段（[scanning.ja.md](scanning.ja.md#フラットフィールド補正と反転)）、より大きな箱が第二の手段です。

<details>
<summary>マージンが足りないと分かったときの、大きな箱への道</summary>

導出すべきは「より大きな箱」であって、「別の種類の箱」ではありません。102 × 126 の窓はそのままに v1 のマージンを与えると、チャンバー内寸は 160 × 181、本体外形はおよそ 164.8 × 185.8 になり、160 × 160 のビルドプレートを超えて 220 クラスが必要になります。天板ステージ、トレイ、シートプレート、アクリルもそれに合わせて拡大します。オープンフロントの構成、ねじなしの重力スタック、積層グリッドはそのまま使えます。ストロボは大きくしなくて済むかもしれません。窓が大きくなる分で約 1.2 段失い、1:1 から 0.25× に下がる分で約 1.3 段戻るからです。ただしこれも測っていません。
</details>
...
`cad/neobox.blend` が、このリポジトリで唯一のジオメトリのソースです。12 の STL ファイルはすべてここから生成されており、blend ファイルを通さずに STL へ届いた変更は、次に誰かが書き出し直した時点で失われます。
...
| シーンの配置 | アセンブリは組み付いた位置のままモデリングしてあり、z の基準は本書と同じです：机（箱の外底面）が z = 0。120 のホルダー 1 組はアセンブリの脇 x = 200 に、6×6 マスクは x = 350 に、スライドプレートは x = 500 に、4×5 ステージは x = 650 に置いてあります |

### コレクション構成

```
Scene Collection
├── mock_view-target    ビューのターゲット用の empty
├── Scaffolding         Camera、Light：レンダリング用の足場
├── NeoBox_v1           筐体、天板ステージ、モックアップ
├── Holder_135          135 のホルダー 1 組。組み付いた位置にある
├── Holder_120          120 のホルダー 1 組。x = 200 に置いてある
├── Masks               6×6 マスク。x = 350 に置いてある
├── Holder_slide        スライドプレートとマウントのモックアップ。x = 500 に置いてある
└── Stage_4x5           4×5 天板ステージとシートプレート、そのモックアップ。x = 650 に置いてある
```
...
|---|---|---|
| `main-body.stl` | `main-body_floor`、`main-body_wall-left`、`main-body_wall-right`、`main-body_wall-rear` | **4 オブジェクト**：底板、左壁、右壁、奥壁。前面が開いているので、まぐさも前柱も要らない |
| `cover-stage.stl` | `cover-stage` | 天板ステージ、一体、窓 62 × 95 |
| `film-holder-135-base.stl` | `film-holder-135-base` | 135 のホルダー底、平底 |
| `film-holder-135-lid.stl` | `film-holder-135-lid` | 135 のホルダー蓋 |
| `film-holder-120-base.stl` | `film-holder-120-base` | 120 のホルダー底、平底 |
| `film-holder-120-lid.stl` | `film-holder-120-lid` | 120 のホルダー蓋 |
| `pressure-window-135.stl` | `pressure-window-135` | 押さえ窓インサート 135 |
| `pressure-window-120.stl` | `pressure-window-120` | 押さえ窓インサート 120 |
| `mask-6x6.stl` | `mask-6x6` | 6×6 マスク |
| `slide-plate-135.stl` | `slide-plate-135` | スライドプレート、1 部品 |
| `cover-stage-4x5.stl` | `cover-stage-4x5` | 4×5 天板ステージ、1 部品 |
| `sheet-plate-4x5.stl` | `sheet-plate-4x5` | 4×5 シートプレート、1 部品 |

### 書き出してはいけないモックアップ

シーンにあるそれ以外のすべては、収まりと光路を見るためのモックアップで、どれ 1 つプリントも書き出しもしません：ストロボ本体とその発光面（`mock_flash-body`、`mock_flash-face`）、T1 レシーバー（`mock_trigger-receiver`）、光路を示す矢印 4 本（`mock_ray-1-into-cavity` から `mock_ray-4-through-window`）、135 と 120 のフィルムストリップのモックアップ（`mock_film-135`、`mock_film-120`）、50 × 50 × 2 のスライドマウント（`mock_slide-mount-50x50x2`）、4×5 のシート（`mock_film-4x5`）、乳白アクリル 2 枚（`mock_diffuser-68x118x2`、`mock_diffuser-106x130x2`）、AN ガラス（`mock_an-glass-64x95x2`）、鉄ワッシャー 4 枚（`mock_steel-shim-1` から `mock_steel-shim-4`）、テキストラベル（`mock_label`）、そしてカメラ／ライト／ビューターゲットという足場（`Camera`、`Light`、`mock_view-target`）です。

見分け方は名前です。プリントするパーツは、自分がなる STL ファイルと同じ名前を持ちます（本体だけは例外で、`main-body_` で始まる 4 枚のシェルに分かれています）。プリントしないオブジェクトは `mock_` で始まります。`Camera` と `Light` だけはこの規則の外ですが、どちらもメッシュではないので STL に届きようがありません。この命名規則は飾りではありません。`tools/export_stl.py` が書き出しのたびに検査していて、`MAPPING` に載っておらず `mock_` でも始まらないメッシュオブジェクトが 1 つでもあれば、書き出しは全体として失敗します。おかげで、シーンに足したプリントパーツを `MAPPING` に書き忘れても、そのパーツの STL が欠けたまま公開されることはありません。

> [!CAUTION]
> **マテリアル名はフィラメントの色を表していません。** マテリアルスロットはレンダリングのためのものです。8 つのマテリアルはいずれも、実際に割り当てられている相手を名前で説明しています：`NB_white` は本体シェルと天板ステージ、`NB_black` は黒でプリントするパーツすべてと、ストロボ本体・ラベル・スライドマウントのモック、`NB_steel` は鉄ワッシャーのモック 4 枚、`NB_opal` は乳白アクリルと AN ガラスのモック、`NB_film` はフィルムのモック 2 枚、`NB_glow` はストロボの発光面、`NB_ray` は光路の矢印 4 本、`NB_receiver` はレシーバーのモックです。プロトタイプ時代に残っていたマテリアルはすべて削除済みで、誤解を招く名前はもう残っていません。それでも色の出典はここではありません。色は必ず[12 点のパーツ](printing.ja.md#12-点のパーツ)から取り、マテリアルスロットからは絶対に取らないでください。
...
| `slide-plate-135.stl` | x 453 – 547、z 0 – 5 に置いてある |
| `cover-stage-4x5.stl` | x 587.6 – 712.4、z 0 – 10 に置いてある |
| `sheet-plate-4x5.stl` | x 594 – 706、z 6 – 11。4×5 天板ステージのデッキの上に立てて置いてある |

スライサーは各ファイルをバウンディングボックスでビルドプレートに落とします。12 ファイルのうち 10 は、読み込んだ時点でプリント面を下にして寝ています。ホルダー蓋の 2 つだけは違います。**上面を下**にしてプリントするパーツなので、インポート後に裏返す必要があります。必要な回転量は、[12 点のパーツ](printing.ja.md#12-点のパーツ)のパーツカードにそれぞれ書いてあります。

### 再書き出しの手順

コマンド 1 本で 12 ファイルすべてを再生成します。これは公開中の STL を作ったパイプラインをリポジトリに収めたものであり、サポートされる唯一の書き出し方法です：
...
5. **コミットの前に検証します。** *チェックポイント：*`python3 tools/verify_stl.py` が `all 12 files pass` と表示し、終了コード 0 で終わること。
...
flowchart LR
  ...
  C -->|12 ファイルすべて合格| D[3 言語のドキュメントを更新]
  C -->|1 つでも FAIL| A
