...
STL は 10 ファイル。白 2 点、黒 8 点、すべてサポート不要です：
...
`cad/neobox.blend` が、このリポジトリで唯一のジオメトリのソースです。10 の STL ファイルはすべてここから生成されており、blend ファイルを通さずに STL へ届いた変更は、次に誰かが書き出し直した時点で失われます。
...
| シーンの配置 | アセンブリは組み付いた位置のままモデリングしてあり、z の基準は本書と同じです：机（箱の外底面）が z = 0。120 のホルダー 1 組はアセンブリの脇 x = 200 に、6×6 マスクは x = 350 に、スライドプレートは x = 500 に置いてあります |
...
シーンにあるそれ以外のすべては、収まりと光路を見るためのモックアップで、どれ 1 つプリントも書き出しもしません：ストロボ本体とその発光面（`mock_flash-body`、`mock_flash-face`）、T1 レシーバー（`mock_trigger-receiver`）、光路を示す矢印 4 本（`mock_ray-1-into-cavity` から `mock_ray-4-through-window`）、135 と 120 のフィルムストリップのモックアップ（`mock_film-135`、`mock_film-120`）、50 × 50 × 2 のスライドマウント（`mock_slide-mount-50x50x2`）、乳白アクリル（`mock_diffuser-68x118x2`）、AN ガラス（`mock_an-glass-64x95x2`）、鉄ワッシャー 4 枚（`mock_steel-shim-1` から `mock_steel-shim-4`）、テキストラベル（`mock_label`）、そしてカメラ／ライト／ビューターゲットという足場（`Camera`、`Light`、`mock_view-target`）です。
...
スライサーは各ファイルをバウンディングボックスでビルドプレートに落とします。10 ファイルのうち 8 つは、読み込んだ時点でプリント面を下にして寝ています。ホルダー蓋の 2 つだけは違います。**上面を下**にしてプリントするパーツなので、インポート後に裏返す必要があります。必要な回転量は、[10 点のパーツ](printing.ja.md#10-点のパーツ)のパーツカードにそれぞれ書いてあります。

### 再書き出しの手順

コマンド 1 本で 10 ファイルすべてを再生成します。これは公開中の STL を作ったパイプラインをリポジトリに収めたものであり、サポートされる唯一の書き出し方法です：
...
5. **コミットの前に検証します。** *チェックポイント：*`python3 tools/verify_stl.py` が `all 10 files pass` と表示し、終了コード 0 で終わること。
...
flowchart LR
  ...
  C -->|10 ファイルすべて合格| D[3 言語のドキュメントを更新]
  C -->|1 つでも FAIL| A
