...
シーンにあるそれ以外のすべては、収まりと光路を見るためのモックアップで、どれ 1 つプリントも書き出しもしません：ストロボ本体とその発光面（`mock_flash-body`、`mock_flash-face`）、T1 レシーバー（`mock_trigger-receiver`）、光路を示す矢印 4 本（`mock_ray-1-into-cavity` から `mock_ray-4-through-window`）、135 と 120 のフィルムストリップのモックアップ（`mock_film-135`、`mock_film-120`）、乳白アクリル（`mock_diffuser-68x118x2`）、AN ガラス（`mock_an-glass-64x95x2`）、鉄ワッシャー 4 枚（`mock_steel-shim-1` から `mock_steel-shim-4`）、テキストラベル（`mock_label`）、そしてカメラ／ライト／ビューターゲットという足場（`Camera`、`Light`、`mock_view-target`）です。
...
> [!CAUTION]
> **マテリアル名はフィラメントの色を表していません。** マテリアルスロットはレンダリングのためのものです。8 つのマテリアルはいずれも、実際に割り当てられている相手を名前で説明しています：`NB_white` は本体シェルと天板ステージ、`NB_black` は黒でプリントするパーツすべてとストロボ本体・ラベルのモック、`NB_steel` は鉄ワッシャーのモック 4 枚、`NB_opal` は乳白アクリルと AN ガラスのモック、`NB_film` はフィルムのモック 2 枚、`NB_glow` はストロボの発光面、`NB_ray` は光路の矢印 4 本、`NB_receiver` はレシーバーのモックです。プロトタイプ時代に残っていたマテリアルはすべて削除済みで、誤解を招く名前はもう残っていません。それでも色の出典はここではありません。色は必ず[9 点のパーツ](printing.ja.md#9-点のパーツ)から取り、マテリアルスロットからは絶対に取らないでください。
