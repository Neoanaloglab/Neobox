...
> [!NOTE]
> This design has not been printed or built. Every row below is a fault the geometry, the materials or the procedure can produce, derived from the design and not from a repair log. Nothing here is a measured result.
...
> [!CAUTION]
> Never add support to force a different orientation. Every part in this build is support-free by design, but only in its stated orientation. The two holder lids go top face down; everything else goes flat base down. Orientation in this project is always described by a feature you can see.
