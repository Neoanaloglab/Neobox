...
三件可选升级另计，从不含在上述总价里：[防牛顿环玻璃](#采购件) CNY 15–30、[USB LED 灯带](#采购件) CNY 10–20、[植绒贴](#采购件) CNY 5–10，三件全买约 CNY 30–60。
...
选闪光灯真正要紧的只有一条：**功率能手动调**。[TTL](glossary.zh-CN.md#ttl) 和 [HSS](glossary.zh-CN.md#hss) 在这里毫无用处，别为它们花钱。v4 的第二条要求"灯头能转 90°"也随着封闭箱体一起消失了：灯平躺着水平直射。

> [!NOTE]
> v4 那条"公开的 STL 只适配 TT560，别的灯都不行"的警告已经不存在了。箱子的任何尺寸都不再取决于闪光灯机身，也就没有长度、厚度限制可查。TT560 留在表里只是作为便宜、好买的参考机型，不是要求；手上有什么手动灯就用什么。
