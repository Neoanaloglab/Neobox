...
NeoBox 是一只 3D 打印的闪光灯光源箱，放在相机下方，用来对 35 mm 和最大 6×9 的 120 底片做[相机翻拍](glossary.zh-CN.md#相机翻拍)（camera scanning，也叫 DSLR scanning），配上可选件还能拍装框的 135 幻灯片和 4×5 页片。组装好的整摞占地 124.8 × 154.8 mm、高 87 mm，放在桌面上用，不占地方。
...
- **12 个打印件**（白色 3 件、黑色 9 件；6×6 遮幅插片、幻灯片板和两个 4×5 件可选），十二个 STL 全部免支撑，都是平面朝下打印。
...
> [!IMPORTANT]
> 这是第一个正式发布版。几何尺寸已在 Blender 源文件中逐项核对，十二个 STL 全部是水密单实体，每一个水平面都落在 0.2 mm 网格上。但这套几何的箱子**从未被打印、拍摄、实测或做过均匀性测试**。你要是动手做了，你就是第一个。

**适合动手的人**：已经在拍胶片，手里有一台能架起来的相机；想要在[基准 ISO](glossary.zh-CN.md#基准-iso)、f/8 下拍摄，并且完全不用担心震动；不介意打印这些件、压几颗磁铁。要拧、要焊、要粘的东西，一样都没有。

**先别开始**：如果你既没有 3D 打印机，也找不到代打服务。所有受力件都是打印出来的，已发布的文件里没有第二条路。4×5 可以装在同一只箱子上拍，但均匀性有待验证，见[常见问题](#常见问题)。
...
![一页纸制造总览：十二个打印文件的尺寸与打印要点、需要采购的零件，以及装配后箱体的剖面](../drawings/manufacturing-overview.zh-CN.svg)
...
| 你买的是什么 | 费用 |
|---|---|
| 全部结构件和箱内之物：十二个 PLA 打印件、乳白亚克力、32 颗磁铁、4 片钢垫片 | 在物料清单中计价 |
...
| 打印 | 十二个 STL，普通 PLA：[层高](glossary.zh-CN.md#层高) 0.2 mm、填充 15%、无支撑 | [3D 打印](printing.zh-CN.md#找代打服务下单) |
...
**支持 4×5 吗？** 支持，靠两个可选件、一块更大的亚克力，外加一条但书。`cover-stage-4x5.stl` 在同一只主箱上代替顶盖台，透光窗 102 × 126 mm，暗槽放一块 106 × 130 × 2 mm 的乳白板；`sheet-plate-4x5.stl` 立在它的托盘里，用 102.2 × 127.6 mm 的片槽接住页片，槽下开 97 × 121 mm 的窗。没有上盖、没有玻璃：4×5 的放大倍率下，景深足以盖过任何卷曲。但书是均匀性：窗口离混光腔壁只有 9 和 12 mm，而 v1 是 29 和 27.5，而且没有测过任何东西，所以先拍一张空白片。见[设计 §10](design.zh-CN.md#10-45-装在-v1-主箱上)和[4×5 页片](scanning.zh-CN.md#45-页片)。

**支持装框的幻灯片吗？** 支持，用可选的幻灯片板。135 的通道高 0.4 mm，是按裸片条的尺寸做的，带框的幻灯片永远进不了胶片夹；改由 `slide-plate-135.stl` 代替胶片夹立在托盘里，它上面一个 51.4 mm 见方的方槽，纸框、塑料框、玻璃框都放得进，槽下是一个 26 × 38 mm 的开窗。没有上盖、没有磁铁：方槽定位，重力压住。见[装框幻灯片](scanning.zh-CN.md#装框幻灯片)。没装框的反转片和普通片条一样走 135 胶片夹。
