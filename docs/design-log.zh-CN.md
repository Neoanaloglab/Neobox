...
**教训：操作动作要按每帧做预算，不能按每次开工做预算。你永远不碰的零件，才是待在原位的零件。**
...
**教训：把调节放在看得见误差的地方。箱子上的螺丝在猜，片平面上的镜子在看。**
...
- **顶盖与胶片台合并为一件——顶盖台**：一块搁在箱壁上的板，托盘、透光窗、亚克力暗槽都在它身上。整套零件里最大的一批就此少了一件，两件之间的接缝也一起消失。
- **亚克力缩为 68 × 118 mm**（v4 是 110 × 130——旧料可以让亚克力店裁小续用）。
- **钢垫片改为四片 10 × 10 mm**，沉在顶盖台台面的沉孔里、与台面齐平。

所有零件仍然免支撑、平面朝下打印。整套九个文件估算 **300–350 g** 耗材——和 v5 的其他数字一样出自估算，但下一张账单会来核对它。

**教训：打印体积是一项规格，不是一个结果。像对待其他尺寸一样，给它一条预算。**
