...
> 怎么把 NeoBox 的十个件打出来：为什么几乎任何打印机都行、一套通用参数、每个文件一张卡片，以及动手装配之前必须做完的检查。

**目录：** [你的打印机打得下吗](#你的打印机打得下吗) · [打印参数](#打印参数) · [耗材](#耗材) · [十个零件](#十个零件) · [先打一副 135 胶片夹](#先打一副-135-胶片夹) · [装配前逐件检查](#装配前逐件检查) · [打紧了或打松了怎么办](#打紧了或打松了怎么办) · [找代打服务下单](#找代打服务下单)

十个 STL 文件：**默认配置 8 个，外加 2 个可选件：6×6 遮幅插片，以及装框幻灯片用的幻灯片板**。两个白色，八个黑色。全部 1:1 毫米，全部免支撑，默认配置的耗材约 **300–350 g，大约是原型机的三分之一**。
...
**靠近胶片的八个件用黑色。** 胶片近距离能「看到」的东西都是黑色，用来吸收杂光：夹底座、夹上盖、压片窗插片、遮幅插片、幻灯片板。

**用量。** 默认配置约 **300–350 g**，大约是原型机的三分之一；可选的幻灯片板另加约 25 g。每种颜色一卷料绰绰有余。
...
就算完全没有打印机，这也已经是一张小单：十个文件（其中两个可选）、约 300–350 g 普通 PLA、全程免支撑，每个件 160 × 160 mm 的打印床都放得下。淘宝搜索词：`3D打印 代打 PLA`。下面这段话原样复制发给店家即可。它点名了每一个文件，并用操作员看得见的特征描述摆放。

```text
共 10 个 STL 文件：必打 8 个，另有 2 个可选（mask-6x6、slide-plate-135）。
单位毫米，1:1，请勿缩放。
