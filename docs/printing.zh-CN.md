...
> 怎么把 NeoBox 的十二个件打出来：为什么几乎任何打印机都行、一套通用参数、每个文件一张卡片，以及动手装配之前必须做完的检查。

**目录：** [你的打印机打得下吗](#你的打印机打得下吗) · [打印参数](#打印参数) · [耗材](#耗材) · [十二个零件](#十二个零件) · [先打一副 135 胶片夹](#先打一副-135-胶片夹) · [装配前逐件检查](#装配前逐件检查) · [打紧了或打松了怎么办](#打紧了或打松了怎么办) · [找代打服务下单](#找代打服务下单)

十二个 STL 文件：**默认配置 8 个，外加 4 个可选件：6×6 遮幅插片、装框幻灯片用的幻灯片板，以及 4×5 顶盖台和片板**。三个白色，九个黑色。全部 1:1 毫米，全部免支撑，默认配置的耗材约 **300–350 g，大约是原型机的三分之一**。
...
**三个壳体件用白色：主箱和两块顶盖台中的任一块。** 主箱内壁就是反射面：闪光灯从完全敞开的前口打进腔内，光在白墙和顶盖台的白色底面之间多次反弹之后才到达胶片。裸露的白色表面*本身*就是光学面。

> [!IMPORTANT]
> 白色请用普通白 PLA，不要丝绸、缎面或其他特殊表面效果的料；内壁保持原样：不喷漆、不打磨、不镀膜。白墙的裸面反射率是光学设计的一部分。

**靠近胶片的九个件用黑色。** 胶片近距离能「看到」的东西都是黑色，用来吸收杂光：夹底座、夹上盖、压片窗插片、遮幅插片、幻灯片板、片板。

**用量。** 默认配置约 **300–350 g**，大约是原型机的三分之一；可选的幻灯片板另加约 25 g，4×5 那两件约 35 g。每种颜色一卷料绰绰有余。
...
| 方槽 | 幻灯片板顶面上那个 51.4 mm 见方的凹槽，装框幻灯片就放在里面 |
| 指槽 | 方槽两侧那两个小盲槽中的一个，让你能把框抠出来 |
| 边缺口 | 片板两条短边上那两个开口切槽中的一个，让你能把页片抠出来 |
...
### [slide-plate-135.stl](../stl/black-pla/slide-plate-135.stl)——可选

- **黑色。** 94 × 120 × 5.0。方槽 51.4 × 51.4，深 2.0；开窗 26 × 38，穿过槽底；两个指槽。
- **在床上：** 平面朝下；方槽和指槽开口朝上。
- **支撑：不需要。** 每个凹槽都开口向上，开窗是通孔。
- **只有在**你有装框的 135 幻灯片时**才打它**：它代替 135 那副胶片夹立在托盘里，框放进方槽。没有上盖、没有磁铁、没有压片元件。

### [cover-stage-4x5.stl](../stl/white-pla/cover-stage-4x5.stl)——可选

- **白色。** 124.8 × 154.8 × 10.0：顶盖台的外形，透光窗改为 102 × 126，暗槽放 106 × 130 × 2 的乳白板，托盘 112.6 × 138.6；没有垫片沉孔。
- **在床上：** 和顶盖台完全一样：带四个角槽的平面朝下，凸起的托盘凸缘朝上。
- **支撑：不需要。** 榫槽顶和顶盖台的一样。
- **只有在**你拍 4×5 时**才打它**：它在同一只主箱上代替顶盖台。它的底面对着混光腔，所以不打磨、不喷漆。

### [sheet-plate-4x5.stl](../stl/black-pla/sheet-plate-4x5.stl)——可选

- **黑色。** 112 × 138 × 5.0。片槽 102.2 × 127.6，深 2.0；开窗 97 × 121，穿过槽底；两条短边上各一个边缺口。
- **在床上：** 平面朝下；片槽和缺口开口朝上。
- **支撑：不需要。**
- **只有在**你拍 4×5 时**才打它**：它立在 4×5 顶盖台的托盘里，页片放进片槽。没有上盖、没有磁铁、没有压片元件。
...
- [ ] 如果打了 `slide-plate-135.stl`：量得 94 × 120，能放进托盘，装框幻灯片放得进它的方槽、也能从指槽抠出来。
- [ ] 如果打了这两件：`cover-stage-4x5.stl` 能像顶盖台一样靠自重落到四个定位榫上，`sheet-plate-4x5.stl` 量得 112 × 138，能放进它的托盘。
...
就算完全没有打印机，这也已经是一张小单：十二个文件（其中四个可选）、约 300–350 g 普通 PLA、全程免支撑，每个件 160 × 160 mm 的打印床都放得下。淘宝搜索词：`3D打印 代打 PLA`。下面这段话原样复制发给店家即可。它点名了每一个文件，并用操作员看得见的特征描述摆放。

```text
共 12 个 STL 文件：必打 8 个，另有 4 个可选（mask-6x6、slide-plate-135、cover-stage-4x5、sheet-plate-4x5）。
单位毫米，1:1，请勿缩放。
...
白色 PLA 2 件 + 1 件可选：
  ...
  - cover-stage-4x5.stl（可选）
    摆放：与 cover-stage.stl 相同。
...
黑色 PLA 6 件 + 3 件可选：
  ...
  - mask-6x6.stl（可选，薄板）
    摆放：平放。
  - slide-plate-135.stl（可选）
    摆放：平面朝下，方槽朝上。
  - sheet-plate-4x5.stl（可选）
    摆放：平面朝下，片槽朝上。
```
