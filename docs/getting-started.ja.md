...
NeoBox は、カメラの下に置いて 135 判と 120 判（最大 6×9）のネガ、そしてオプション部品でマウント済みの 135 スライドと 4×5 のシートを撮影する、すなわち[カメラスキャン](glossary.ja.md#カメラスキャン-camera-scanning) (camera scanning) するための、3D プリント可能なストロボ光源ボックスです。組み上がった状態で設置面積 124.8 × 154.8 mm、高さ 87 mm。床ではなく、机の上に収まるサイズです。
...
- **プリントパーツ 12 点**（白 3 点・黒 9 点。6×6 マスク、スライドプレート、4×5 の 2 点はオプション）。STL 12 ファイルはすべてサポート不要で、平らな面を下にして出力します。
...
> [!IMPORTANT]
> これは最初の公開リリースです。形状は Blender ソース上で寸法検証済みで、STL 12 ファイルはいずれも水密な単一ソリッド、すべての水平面が 0.2 mm グリッドに乗っています。ただしこの形状の箱は、**まだ一度も出力も撮影も実測も均一性テストもされていません。** あなたが作れば、あなたが最初の 1 人です。

**作るべき人。** すでにフィルムで撮っていて、スタンドに載せられるカメラを持っている人。振動が問題にならない[ベース ISO](glossary.ja.md#ベース-iso) (base ISO)・f/8 の撮影がしたい人。パーツの出力とマグネットの圧入が苦にならない人。締める・はんだ付けする・接着する作業は一切ありません。

**やめておくべき人。** 3D プリンタも出力サービスも使えない人。構造部品はすべてプリントで、公開ファイルに代替ルートはありません。4×5 は同じ本体で可能ですが、均一性には留保があります。[よくある質問](#よくある質問)を参照してください。
...
![製造総図：プリントする 12 ファイルの寸法と出力上の注意、購入部品、そして組み上がった箱の断面](../drawings/manufacturing-overview.ja.svg)
...
| 何を買うのか | 費用 |
|---|---|
| 構造にかかわるすべて：PLA プリントパーツ 12 点、乳白アクリル、マグネット 32 個、鉄ワッシャー 4 枚 | 部品表に記載 |
...
| 出力 | STL 12 ファイルを普通の PLA で出力。[積層ピッチ](glossary.ja.md#積層ピッチ-layer-height) 0.2 mm、インフィル 15%、サポートなし | [出力](printing.ja.md#出力サービスへの発注) |
...
**4×5 は使えますか。** はい。オプションの部品 2 点と一回り大きいアクリル、それに留保が 1 つ付きます。`cover-stage-4x5.stl` が同じ本体の上で天板ステージの代わりに載り、102 × 126 mm の採光窓と、106 × 130 × 2 mm の乳白板を収める溝を持ちます。`sheet-plate-4x5.stl` はそのトレイに立ち、97 × 121 mm の窓の上の 102.2 × 127.6 mm のポケットでシートを受けます。蓋もガラスもありません。4×5 の撮影倍率では、被写界深度が反りをまるごと吸収します。留保は均一性です。窓はチャンバーの壁から 9 と 12 mm で、v1 の 29 と 27.5 より近く、何も測っていません。まず何も入れずに 1 コマ撮ってください。[設計 §10](design.ja.md#10-v1-本体での-45)と[4×5 シート](scanning.ja.md#45-シート)を参照してください。

**マウント済みスライドは使えますか。** はい、オプションのスライドプレートで使えます。135 用の通路は高さ 0.4 mm、素のフィルムストリップ用の寸法なので、マウントがホルダーに入ることはありません。代わりに `slide-plate-135.stl` がホルダーの位置にトレイに立ちます。51.4 mm 角のポケットが紙・プラスチック・ガラスのマウントを受け、その下に 26 × 38 mm の窓があります。蓋もマグネットもなく、ポケットが位置を決めて重力が押さえます。[マウント済みスライド](scanning.ja.md#マウント済みスライド)を参照してください。マウントなしのポジフィルムは、ほかのストリップと同じく 135 ホルダーに通します。
