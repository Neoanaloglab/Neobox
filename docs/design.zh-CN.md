...
**目录：**[1. 光路](#1-光路) · [2. 尺寸链](#2-尺寸链) · [3. 光学决策](#3-光学决策) · [4. 顶盖台](#4-顶盖台) · [5. 胶片夹](#5-胶片夹) · [6. 壳体](#6-壳体) · [7. 对焦灯](#7-对焦灯) · [8. 闪光灯用法](#8-闪光灯用法) · [9. 复位与重复性](#9-复位与重复性) · [10. 4×5 装在 v1 主箱上](#10-45-装在-v1-主箱上) · [11. 使用源文件](#11-使用源文件)
...
敞开前口就是 v1 的全部架构变化。打印过的原型机（内部第 4 版）把闪光灯封在箱内，它的一切麻烦都由此而来：箱体高度要从灯体厚度推导、要开抽口才够得着出力拨盘、要留过线孔穿线，发布的 STL 也只适配一款闪光灯。灯一出箱，这些全部消失：任何品牌的闪光灯都能用，接收器留在桌面上，信号干净、换电池不碰箱子，抽口、盖板、过线孔和通风问题都不复存在（[§6](#6-壳体)）。代价是环境光可以进入混光腔。这个交换是算过账的，不是装看不见：在同步速度下，闪光脉冲远比室内环境光亮（[§8](#8-闪光灯用法)）。
...
原型机的箱子是从闪光灯推导出来的，那些数字也随它一起作废了。v1 切断了这条线：闪光灯根本不进箱，尺寸链不再从某款产品的规格表开始，而是从桌面开始，一件叠一件，靠重力向上生长。

### 打印件

十二个 STL 文件，白色三件、黑色九件，全部免支撑：
...
| `slide-plate-135.stl` | 黑 | 94 × 120 × 5 | 装框 135 幻灯片用的幻灯片板：51.4 × 51.4 方槽、26 × 38 开窗、两个指槽；独立的第四副夹子，只有一件（[§5](#5-胶片夹)） |
| `cover-stage-4x5.stl` | 白 | 124.8 × 154.8 × 10.0 | 4×5 顶盖台：沿用顶盖台的外形、板身和四个榫槽，透光窗改为 102 × 126、亚克力暗槽 107 × 131、托盘 112.6 × 138.6；没有垫片沉孔（[§10](#10-45-装在-v1-主箱上)） |
| `sheet-plate-4x5.stl` | 黑 | 112 × 138 × 5 | 4×5 片板：102.2 × 127.6 片槽、97 × 121 开窗、两个边缺口；立在 4×5 顶盖台的托盘里 |
...
| 夹底座 | 79.0 – 84.0 | 立在凸缘围出的台面上；幻灯片板占的也是这 5 mm |
| **胶片面** | **83.2** | 承台顶面，两种画幅同一高度 |
| 幻灯片的胶片面 | ≈ 82.6 – 83.6 | 在幻灯片板里：槽底 82.0 加上框厚的一半左右；重新对焦 |
| 4×5 的胶片面 | ≈ 82.2 | 页片平躺在片板槽底 82.0 上；重新对焦 |
...
没有任何东西需要重新推导。原型机那套把闪光灯规格表换算成箱体高度的公式没有了，因为输入本身没有了：**v1 箱体的任何尺寸都不编码任何闪光灯的任何尺寸。** 替代的闪光灯只需要能手动调出力、灯头能平躺着水平对准敞开前口。TT560 的发光面 60 × 45、中心离桌 31，是参考，不是限制。接收器同样在箱外，触发器的型号也同样自由。换闪光灯既不用动 `cad/neobox.blend`，也不用动 STL（[§11](#11-使用源文件)）。
...
| 插片／遮幅片 | 平板，2.0 ／ 1.0 |
| 幻灯片板 | 0 · 1.0 指槽底 · 3.0 槽底 · 5.0 顶面 |
| 4×5 顶盖台 | 0 · 2.8 榫槽顶 · 3.6 亚克力槽底 · 6.0 台面 · 10.0 凸缘顶 |
| 片板 | 0 · 1.0 缺口底 · 3.0 槽底 · 5.0 顶面 |

这张网格的意义：在 0.2 mm [层高](glossary.zh-CN.md#层高)下，上面每一个特征位都正好落在层边界上，打印出来的 0.4 台阶就是真正的 0.4，而不是切片软件四舍五入的产物。0.1 mm 也整除这张网格；0.12 和 0.16 不整除。这套推理源自原型机的胶片夹，原样沿用（[设计日志第 18 条](design-log.zh-CN.md#18-夹子按层高量化)）；v1 的新意在于整个设计都服从它，而不只是夹子。下单打印规格是所有件一律 0.2（[打印文档](printing.zh-CN.md)）。校验脚本对每次导出都强制检查网格和最小台阶（[§11](#11-使用源文件)）。
...
顶盖台是原型机的顶盖和胶片台合并成的一块白色板：过去的一个盖、一块台、三根螺杆、六颗螺母、三颗热熔铜螺母，现在是一个打印件。它直接搁在箱壁顶上，承载混光腔以上的全部光学件。

这一个零件身上带着什么：

- **板体**：124.8 × 154.8，总高 10；结构板厚 6 mm，台面落在 z = 79.0。四角榫槽深 2.8，罩住主箱 2.6 高的定位榫，竖向留 0.2 间隙。板搁在壁顶上，永远不压在榫上；榫只负责 XY 定位。
- **透光窗**：62 × 95，居中，贯穿板体。
...
- **垫片沉孔**：四个，10.6 见方、深 1.0，位于 (±41, ±12)。每孔放入一片 10 × 10 × 1 钢垫片（或 Ø10 × 1 圆片），与台面齐平：给夹底座的磁铁当铁座，夹子啪地吸落在托盘上不再乱动，干的和原型机那四片粘上去的垫片是同一件事，只是不再用胶。

要挪动它，捏住凸缘一提，整个顶盖台一体离箱。

**没有调平五金，这是设计本身，不是缺漏。** 原型机用三根 M6 螺杆把胶片台对着箱体调平；v1 把螺杆、螺母、铜螺母连同那套流程一起删掉了。

<details>
<summary>为什么调平挪到了相机端</summary>

这个设计里塑料从不承担螺纹。FDM 打印的螺纹既弱又会蠕变；原型机已经在用铜螺母加钢螺杆回避它，v1 则连这些都不再需要。

更深一层的原因是：唯一要紧的平行度是**胶片面对传感器**，而对着箱体调平胶片台从来就没有直接给出过这件事；原型机拧完三颗螺母，还是得把相机对着台面摆正。v1 只保留了真正要紧的那一步。在胶片台上放一面小镜子，看着取景器移动相机，直到镜头自己的倒影居中：居中的那一刻，传感器就平行于镜面，也就平行于镜子所躺的胶片面（[装配文档的镜子法](assembly.zh-CN.md#第-7-步相机端调平镜子法)）。
...
每种画幅一副夹子：夹底座加夹上盖，都是 94 × 120，立在顶盖台的托盘里。换画幅就是掀走一副、放下另一副：磁吸松开再吸合，前后五秒，其余什么都不动。过片是把片条从合着的夹子里抽过去，整卷中途从不开夹。装框的 135 幻灯片另有第四副，只有一块板、没有上盖（[见下](#装框幻灯片幻灯片板)）。
...
装框的幻灯片厚约 1.2 – 3.2 mm，是 0.4 mm 通道的许多倍，永远进不了 135 夹；而在 135 底座上给它挖一个槽，又会把托住片条的承台挖掉（[设计日志第 25 条](design-log.zh-CN.md#25-装框幻灯片自成一板)）。所以 `slide-plate-135.stl` 是独立的第四副夹子，只有一件：一块 94 × 120 × 5 的黑色板，代替 135 那副立在托盘里。它没有上盖、没有磁铁、没有压片元件：幻灯片框本身是硬的，箱子又是卧式的，方槽负责定位，重力负责压住。

| 特征 | 幻灯片板 |
|---|---|
| 外形与厚度 | 94 × 120 × 5，在托盘里坐的是和夹底座同一个位置 |
| 方槽 | 51.4 × 51.4，深 2.0；槽底在板底以上 3.0（z = 82.0） |
| 开窗 | 26 × 38，穿过槽底 |
| 指槽 | 两个，18 × 12，居中开在方槽的两条短边上；底在 1.0，不穿透 |
| 胶片面 | 槽底加上框厚的一半左右：纸框约 82.6，玻璃框约 83.6；重新对焦 |

**为什么是 51.4：** 幻灯片框标称 50 × 50，但纸框可以到 50.8（2 英寸），51.4 给最大的框每边留 0.3 mm、给塑料框每边留 0.7 mm，和夹底座在托盘里的间隙是同一口径。**为什么深 2.0：** 最薄的纸框约 1.2，仍然被槽壁扣住；最厚的玻璃框约 3.2，露出 1.2，上面没有任何东西会碰到它。**为什么是 26 × 38 而不是 25 × 37：** 框在槽里最多能偏 0.7 mm，相对标称的 24 × 36 画幅每边留 1 mm 余量，框住画面的就永远是框自己的片窗，而不是打印出来的边；任何框在四边仍然比开窗多盖出 5 mm 以上，所以不漏光。**指槽**让指甲能伸到纸框的边下面，纸框比槽沿低 0.8；指槽是盲槽，比槽底再低 2.0，不透光。

装框幻灯片的放法和你拿着它对着窗看时一样：从上往下看图像是正的，这样药膜就朝下，和片条同一规则。翘曲的纸框或者鼓起来的片子，这里没有任何东西能压平，专用的幻灯片扫描仪也一样；1:1、f/8 下约 ±0.4 mm 的景深能吃掉大部分。

---

## 6. 壳体

**主箱**是一体打印件：底板 3.0，左、右、后三面壁 2.4，前面根本没有墙。侧壁顶上立着四个定位榫，2.4 × 12 × 2.6，与顶盖台的四角榫槽配合。

**敞开前口删掉了什么。** 原型机需要抽口盖板，因为闪光灯的出力拨盘封在不透光的箱子里；需要过线孔，因为线要穿过一面遮光墙；还要回答通风问题，因为热量无处可去。v1 的箱内没有任何需要伸手去够的东西：闪光灯、接收器和它们的电池都在桌面上，改出力抬手就到，对焦灯的线直接从箱口走出去，腔体本身就通着空气。三个问题和那面前墙一起消失了。
...
> [!NOTE]
> **整台装配的紧固件清单是：零颗螺丝、零滴胶水。** 连原型机仅剩的两处用胶（夹子磁铁和铁垫片）也没有了：v1 的磁铁是过盈压入的，垫片是落在沉孔里的。
...
箱内没有内置对焦灯，也没有任何电气件。取景和对焦用普通房间光就够：昏暗房间里看清胶片细节绰绰有余，什么灯都行，只要别直射敞开前口；拍摄瞬间闪光会把房间光全部压过去。
...
**两种片条画幅**的胶片面都固定在 z = 83.2（6×6 遮幅片把 120 那副抬高 1 mm；幻灯片板和片板托出的胶片面与它相差不到一毫米；重新对焦即可，其余不变）。135 和 120 之间变的是需要的放大倍率，因而变的是相机高度，不是胶片的高度。相机高度要用胶片面高度加上镜头的[工作距离](glossary.zh-CN.md#工作距离)算出来，算法在[翻拍扫描的相机高度与支架](scanning.zh-CN.md#相机高度与支架)。
...
4×5 靠两个可选件和一块更大的亚克力，直接装在原封不动的主箱上：`cover-stage-4x5.stl` 代替顶盖台，`sheet-plate-4x5.stl` 立在它的托盘里，一块 106 × 130 × 2 的乳白板躺在它的暗槽里。其余什么都不变：同一组定位榫，同一支闪光灯放在同一个敞开前口，同一套重力堆叠，同一块 160 × 160 的打印床。

| 特征 | 4×5 顶盖台 | v1 顶盖台 |
|---|---|---|
| 外形、板身、榫槽 | 124.8 × 154.8，6 mm 板，四个榫槽对准定位榫 | 相同 |
| 透光窗 | 102 × 126 | 62 × 95 |
| 亚克力暗槽 | 107 × 131，槽底在 3.6，放 106 × 130 × 2 的板 | 69 × 119，放 68 × 118 × 2 |
| 凸缘围出的托盘 | 112.6 × 138.6，凸缘宽 4，顶在 10.0 | 94.6 × 120.6，凸缘宽 7 |
| 垫片沉孔 | 没有：片板没有磁铁 | 四个 |
| 混光余量，窗口边到箱壁 | x 方向 9，y 方向 12 | x 方向 29，y 方向 27.5 |

| 特征 | 片板 |
|---|---|
| 外形与厚度 | 112 × 138 × 5，立在 4×5 凸缘围出的台面上 |
| 片槽 | 102.2 × 127.6，深 2.0；槽底在板底以上 3.0（z = 82.0） |
| 开窗 | 97 × 121，穿过槽底：96 × 120 的影像区加上惯例的每边 0.5 |
| 边缺口 | 两个，宽 18，开在两条短边上，从页片边缘下方 3.7 处一直切出板边；底在 1.0 |
| 胶片面 | 页片平躺在槽底上：约 82.2 |

**为什么页片不需要压片元件。** 全画幅拍 4×5 约为 0.25×，f/8 下的景深大约 8 – 10 mm，是 135 的十倍。页片自身一两毫米的卷曲在这个尺度上看不出来，所以片板就是一个槽，页片边缘下面有承台，上面什么都没有：没有上盖、没有玻璃、没有磁铁。页片一张一张装，从上往下看图像是正的（药膜朝下），从边缺口抠出来。

**不知道的事：均匀性。** v1 的窗口离箱壁 29 和 27.5 mm，4×5 的窗口只离 9 和 12。从来没有确立过最小余量，也没有测过任何东西，所以 4×5 套件是作为同一只箱子上的实验发布的，不是作为一个数字。先拍一张空白片：如果出现你不能接受的渐变，第一个办法是后期平场校正（[scanning.zh-CN.md](scanning.zh-CN.md#平场校正与反相)），第二个办法是更大的箱子。

<details>
<summary>余量真的不够时的大箱路线</summary>

要推导的是一只更大的箱子，不是另一种箱子：保留 102 × 126 的窗口，给它 v1 的余量，内腔就成了 160 × 181，主箱外形约 164.8 × 185.8，超出 160 × 160 的床、要上 220 级的床，顶盖台、托盘、片板和亚克力按比例跟着改。敞开前口的架构、无螺丝的重力堆叠和层格原样沿用。闪光灯未必要换大：更大的窗口要多吃约 1.2 档，而从 1:1 降到 0.25× 又还回约 1.3 档，不过这一点同样没有实测。
</details>
...
`cad/neobox.blend` 是本仓库里唯一的几何来源。十二个 STL 文件都由它生成，任何绕过 blend 文件、直接改到 STL 上的改动，下一次有人重新导出时就会丢失。
...
| 场景布局 | 装配体就位建模，z 基准与本文一致：桌面（也就是箱体外底面）为 z = 0。120 那副夹子停放在装配体旁 x = 200 处，6×6 遮幅片停放在 x = 350 处，幻灯片板停放在 x = 500 处，4×5 套件停放在 x = 650 处 |
...
├── Holder_slide        幻灯片板和它的幻灯片框示意件，停放在 x = 500
└── Stage_4x5           4×5 顶盖台和片板及其示意件，停放在 x = 650
```
...
| `slide-plate-135.stl` | `slide-plate-135` | 幻灯片板，一体 |
| `cover-stage-4x5.stl` | `cover-stage-4x5` | 4×5 顶盖台，一体 |
| `sheet-plate-4x5.stl` | `sheet-plate-4x5` | 4×5 片板，一体 |

### 哪些是示意件，绝不能导出

场景里其余的一切都是看装配关系和光路用的示意件，一件也不打印、不导出：闪光灯灯体和它的发光面（`mock_flash-body`、`mock_flash-face`）、T1 接收器（`mock_trigger-receiver`）、四支光路箭头（`mock_ray-1-into-cavity` 到 `mock_ray-4-through-window`）、135 和 120 的胶片示意条（`mock_film-135`、`mock_film-120`）、一个 50 × 50 × 2 的幻灯片框（`mock_slide-mount-50x50x2`）、一张 4×5 页片（`mock_film-4x5`）、两块乳白亚克力（`mock_diffuser-68x118x2`、`mock_diffuser-106x130x2`）、防牛顿环玻璃（`mock_an-glass-64x95x2`）、四片钢垫片（`mock_steel-shim-1` 到 `mock_steel-shim-4`）、一个文字标签（`mock_label`），以及相机／灯光／视图目标这些脚手架（`Camera`、`Light`、`mock_view-target`）。
...
> [!CAUTION]
> **材质名不代表耗材颜色。** 场景里现在只有八个材质，全部只为渲染服务，名字说的是它贴在什么东西上：`NB_white` 贴主箱四片壳体和顶盖台，`NB_black` 贴全部黑色打印件外加闪光灯灯体、文字标签和幻灯片框这三个示意件，其余六个（`NB_steel`、`NB_opal`、`NB_film`、`NB_glow`、`NB_ray`、`NB_receiver`）只贴在示意件上。原型机时代那批材质已经全部删除，槽里不再有会误导人的残余。即便如此，耗材颜色仍然一律以[打印文档的十二个零件](printing.zh-CN.md#十二个零件)为准，绝不要看材质槽。
...
| `slide-plate-135.stl` | 停放在 x 453 – 547，z 0 – 5 |
| `cover-stage-4x5.stl` | 停放在 x 587.6 – 712.4，z 0 – 10 |
| `sheet-plate-4x5.stl` | 停放在 x 594 – 706，z 6 – 11，立在 4×5 顶盖台的台面上 |

切片软件是按包围盒把文件落到打印床上的。十二个文件里有十个导入时就已经打印面朝下躺平；两个夹上盖不是：它们要**顶面朝下**打印，导入后必须翻面。每个零件需要转多少，写在[打印文档的十二个零件](printing.zh-CN.md#十二个零件)那一节的零件卡片上。

### 重新导出

一条命令再生成全部十二个文件。它就是产出已发布 STL 的那套管线的入库版本，也是唯一受支持的导出方式：
...
5. **提交之前先校验。** *检查点：*`python3 tools/verify_stl.py` 打印 `all 12 files pass` 并以 0 退出。
...
在原型机时代，这个标题下面是一份重新推导的清单。到了 v1，没有任何东西需要改：**箱体的任何尺寸都不编码闪光灯**，换一只闪光灯（或者换一套触发器），既不碰 `cad/neobox.blend` 也不碰 STL。新闪光灯只要能手动调出力、灯头能平躺着水平对准敞开前口；换完重拍一张测光试片，继续干活（[§8](#8-闪光灯用法)）。

### `cad/` 里的其他文件

- `film-stage-aluminium-3mm.dxf`：原型机的铝制胶片台，只作历史保存。v1 没有胶片台：这个零件已并入顶盖台，现在的设计里没有任何东西按它切。
