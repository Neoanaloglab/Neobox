...
| プリント代行へ送れるバンドル（内容はまだ前リビジョンのままで、v5 用に再構築中） | [工具と消耗品](docs/bom.ja.md#工具と消耗品)に挙げたもの——組立自体にはドライバーもレンチもはんだごても不要です |
...
**LED パネルも検討したうえで見送りました。** パネルはそれ自体が面光源ですが、上に述べた実効シャッターの利点が失われます。判断の根拠は[設計](docs/design.ja.md#8-ストロボの運用)と [FAQ](docs/getting-started.ja.md#よくある質問) にあります。
...
*（上の図はまだ前リビジョンの内容です。図面一式は v5 用に再生成中です。）*
...
| [`drawings/`](drawings/) | 光路図、断面図、印刷姿勢、分解図、撮影セットアップ、製造総図を 3 言語分——内容はまだ前リビジョンのままで、再生成待ち | 生成物 |
| [`docs/`](docs/) | ドキュメント一式。各ファイルに English・简体中文・日本語 | ソース |
| [`taobao-order/`](taobao-order/) | 中国のプリント代行向けバンドル——内容はまだ前リビジョンのままで、v5 用に再構築中 | 生成物 |
