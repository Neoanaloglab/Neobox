...
![许可证：MIT](https://img.shields.io/badge/许可证-MIT-blue) ![状态：原型，尚未打印](https://img.shields.io/badge/状态-原型-orange) ![支持画幅：135 与 120](https://img.shields.io/badge/画幅-135%20%C2%B7%20120-lightgrey) ![外形尺寸约 125 × 155 × 83 mm](https://img.shields.io/badge/尺寸-约%20125%20%C3%97%20155%20%C3%97%2083%20mm-lightgrey)
...
| 采购件 | 1 块乳白亚克力扩散板、32 颗 Ø8 × 2 mm 磁铁、4 片钢垫片；可选：1 片防牛顿环玻璃、1 条 USB LED 灯带、1 张植绒贴 |
...
**曝光是那一记闪光脉冲完成的。** 画面由工作功率下持续 1/1,000–1/20,000 秒的一记脉冲定义。箱子前面是敞开的，环境光*可以*进入混光腔——但脉冲远比室内光亮得多。在昏暗的房间里拍摄、别让顶灯直射箱口，环境光的贡献就小到看不见。相机快门要做的，只是在脉冲发生时保持打开。
...
**第二个论据在光谱域——胶片在这里比看上去挑剔得多。** 彩色负片是三层叠置的染料——黄、品红、青，分别记录蓝、绿、红；其中青色层和橙色罩色掩膜都工作在光谱的红端。扫描光源必须同时在三个波段给足能量：光源在哪里有缺口，哪一层染料就被欠采样，三个通道就无法干净分离——这笔账会在反相之后送到：颜色曲线交叉、顽固的偏色，靠一次白平衡是修不回来的。

**氙气闪光管是卤化银材料的"原生"光源。** 它发出真正连续的光谱，色温约 5600 K，显色达到日光级（CRI 典型 ≥ 95），深红充足——正是 LED 普遍偏弱的 R9 色块。典型白光 LED 则是蓝色泵浦加荧光粉：450 nm 附近一根尖峰、480 nm 上下一个凹谷（"青色谷"）、荧光粉的宽包之后从约 630 nm 起滚降——恰好砸在青色层和橙色掩膜最需要光的地方。下图画出了问题的形状；曲线为两类光源的典型示意，非实测。

![闪光灯与 LED 的光谱与时间域对比](drawings/spectral-response.zh-CN.svg)

**用三色 LED 合成的"白光"同样不行——原因正好相反。** RGB 混光的光谱是三根窄尖峰，峰与峰之间几乎空无一物：染料光谱里成段的波段根本从未被采样。单张拍摄的彩色相机会让事情更糟——它的颜色矩阵是按连续光源标定的，三根尖峰穿过拜耳滤色片会产生标定从未预期的通道串扰，而混光配比里焊死的白点又和橙色掩膜相冲。结果是随片基而变、怎么调都调不干净的偏色。

**理论上的最优解要坦率承认：黑白相机加 RGB 分次曝光。** 在没有拜耳阵列的传感器上，用窄带红、绿、蓝各曝光一次再合成，就是密度计级的三色分离扫描——每个通道都是全分辨率，通道分离由光源而不是滤色片染料定义。这是鼓式扫描仪的一脉；代价是小众的黑白机身、每格三次曝光和一套对齐流程。NeoBox 刻意站在这条曲线的务实点位上：一闪、一张、任何相机——再用连续光谱让单张路线保持诚实。

**第三个论据安静但重要：一致性。** 手动闪光在固定功率档位下逐张高度可重复：整卷每一张接受的光完全相同，一套反相配置通吃全卷。没有预热漂移、整场拍摄没有热致色移、也没有 PWM 调光——电子快门下不会出现条带。而这一切是在 f/8、[基准 ISO](docs/glossary.zh-CN.md#基准-iso)（base ISO）下拿到的，功率还有富余。

**这个选择有商业先例。** 理光的 [PENTAX FILM DUPLICATOR](https://ricohimagingstore.com/pentax-film-duplicator.html)——翻拍领域的标杆设备、出自胶片血统的相机厂——官方配置就是这一套配方：数码相机加微距镜头，外接闪光灯隔着扩散屏打光。NeoBox 用的是同一套光学配方，只是把光箱变成在家就能打印的东西，再配上自己的走片系统。

**LED 灯板方案考虑过，被否决了。** 灯板本身就已经是一个发光面，但它同时拿不到上面的等效快门论据和光谱论据。完整推理见[设计](docs/design.zh-CN.md#8-闪光灯用法)和[常见问题](docs/getting-started.zh-CN.md#常见问题)。
...
3. **平滑交给一块乳白亚克力扩散板。** 一块 68 × 118 × 2 mm 的扩散板嵌在顶盖台的浅槽里、位于 62 × 95 mm 透光窗之下、胶片下方 4.6 mm。它上面的灰尘不会成像——因为扩散板本身就是那个发光面——偶尔擦一擦就行。
...
NeoBox 之前的版本把闪光灯封在箱体里，箱子必须围着所支持的最大号闪光灯来定尺寸，换电池也得开箱。v5 把前壁整个去掉：闪光灯躺在桌面上向箱内发光，壳体从此不依赖任何闪光灯尺寸，任何品牌都能用，无线接收端也留在信号最通畅的箱外。
...
- [ ] **2. 采购** —— 一块裁成 68 × 118 × 2 mm 的乳白亚克力扩散板、32 颗 Ø8 × 2 mm N35 磁铁、4 片钢垫片；可选一片 64 × 95 × 2 mm 防牛顿环玻璃、一条 USB LED 灯带和一张植绒贴。→ [物料清单](docs/bom.zh-CN.md#工具与耗材)
...
| [`cad/film-stage-aluminium-3mm.dxf`](cad/film-stage-aluminium-3mm.dxf) | v4 的铝制胶片台——已废弃；v5 中它的功能并入顶盖台 | 历史存档 |
