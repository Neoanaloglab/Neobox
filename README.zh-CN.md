...
> 一台开源硬件、全部零件可 3D 打印的桌面闪光翻拍光箱，用于相机翻拍 135 与 120 胶片：外形约 125 × 155 × 83 mm，12 个打印件（其中 4 个可选）约 300–350 g，全程无螺丝、无螺纹、无工具，纯磁吸组装。任意热靴闪光灯平放在敞开前口就能用；压片插片平台负责压平胶片，可选一片通用防牛顿环玻璃升级。
...
因为闪光灯留在箱体之外，箱子只需要装下混光腔和胶片平台，所以 12 个打印件、约 300–350 g 的 PLA、一块 160 × 160 mm 的打印床就够了。

| 项目 | 规格 |
|---|---|
| 外形尺寸 | 约 125 × 155 × 83 mm（占地 124.8 × 154.8 mm；放上胶片夹后总高 87 mm） |
| 支持画幅 | 135 与 120，最大到 6×9；6×6 用遮幅插片；6×4.5 靠后期裁切；装框的 135 幻灯片用可选的幻灯片板；4×5 页片用可选的 4×5 套件，均匀性有待验证 |
| 打印件 | 12 个 STL 文件（白 3 件、黑 9 件，含可选的 6×6 遮幅插片、幻灯片板和 4×5 套件），约 300–350 g，全部免支撑 |
...
| 胶片夹 | 135 与 120 两副滑槽式胶片夹，用 Ø8 × 2 磁铁闭合；换幅面就是掀掉一副、放上另一副；装框的 135 幻灯片另有一块带方槽的幻灯片板 |
...
| 本仓库提供 | 你必须自备或另行购买 |
|---|---|
| 12 个 STL 文件（白 3 件、黑 9 件），每一件都平面朝下、免支撑打印 | 一台支持手动曝光和 raw 拍摄的相机（微单或单反皆可） |
...
- [ ] **3. 打印**：12 个 STL 文件、两种颜色（白 3 黑 9），普通 PLA，免支撑；每一件都平面朝下打印。白色耗材必须是**哑光**的：丝绸白或亮面白会在腔内留下镜面反光。→ [3D 打印](docs/printing.zh-CN.md#3d-打印)
...
| [`stl/black-pla/`](stl/black-pla/) | 四个胶片夹零件、两个压片窗插片、`mask-6x6.stl`、`slide-plate-135.stl` 和 `sheet-plate-4x5.stl` | 生成物 |
| [`drawings/`](drawings/) | 光路、剖面、打印方向、爆炸图、拍摄布置和制造总图，三种语言，由 `tools/drawings/` 里的生成器生成 | 生成物 |
| [`docs/`](docs/) | 文档集，每份都有英文、简体中文和日文三个版本 | 源文件 |
| [`tools/export_stl.py`](tools/export_stl.py) | 随仓库提交的导出脚本，从 `cad/neobox.blend` 重新生成十二个 STL 文件 | 源文件 |
...
- **已验证：** 十二个 STL 文件全部是水密的单一实体，非流形边为零；在各自的打印方向上，每一个水平面都落在 0.2 mm 的层栅格上；每个包围盒都与公布的尺寸一致。运行 `tools/verify_stl.py` 可以复现这些结果。
